...
(function() {
	...
	/**
	 * constant for detecting if we are at the bottom of the page
	 */
	...
	Ixxus.DocumentListEndlessScrollRenderer = function(name) {
		Ixxus.DocumentListEndlessScrollRenderer.superclass.constructor.call(
				this, name);
		...
		this.loadingDiv.innerHTML = Alfresco.util.message(
				"endless.loadingMessage", "Alfresco.DocumentList");
		//need to setup the function to be called
		this._handleScroll = function DL__handleScroll(e) {
			var scrollPos;
			if (this.updateInitiated
					|| this.options.viewRendererName !== "endless") {
				return;
			}
			...
			if ((pageHeight - (scrollPos + clientHeight) < PIXELS_FROM_BOTTOM)
					&& this.currentPage < this.widgets.paginator
							.getTotalPages()) {
				...
				Alfresco.logger
						.debug("scrolled to end firing the paginator setPage to "
								+ nextPage);
				//fire the pagination to the next page and display loading message
				this.widgets.dataTable
						.appendChild(this.viewRenderers.endless.loadingDiv);
	 ...
	 */
	YAHOO.extend(Ixxus.DocumentListEndlessScrollRenderer,
	 ...
	 */
	Ixxus.DocumentListEndlessScrollRenderer.prototype.renderView = function Ixxus_Endless_DL_VR_renderView(
		...
		if (scope.updateInitiated) {
			Alfresco.logger
					.debug("append dataTable and set updateInitiated to false");
			//append rows instead of replacing the table
			scope.widgets.dataTable.onDataReturnAppendRows.call(
					scope.widgets.dataTable, sRequest, oResponse, oPayload);
			//remove the loading message
			scope.widgets.dataTable
					.removeChild(scope.viewRenderers.endless.loadingDiv);
			//reset the flag that indicates we are updating via a scroll
			scope.updateInitiated = false;
		} else {
			Alfresco.logger.debug("InitializeTable ");
			//+ scope.currentFilter.filterId + " " + scope.currentFilter.filterData);
			//if this is not the first page we need to change to the first page
			//there is a bug in Alfresco 4.0.1 and alfresco 4.0.2 you may notice when navigating between folders https://issues.alfresco.com/jira/browse/ALF-14956
			if (scope.currentPage && parseInt(scope.currentPage, 10) !== 1) {
	 ...
	 * @param oPayload {MIXED} (optional) Additional argument(s)
	 */
	Ixxus.DocumentListEndlessScrollRenderer.prototype.destroyView = function DL_VR_destroyView(
			scope, sRequest, oResponse, oPayload) {
		...
		if (scope.options.viewRendererName !== "endless") {
			Alfresco.logger
					.debug("set updateInitiated to false and remove scroll handler");
			scope.updateInitiated = false;
